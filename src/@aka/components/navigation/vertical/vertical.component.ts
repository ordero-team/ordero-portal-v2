...
@Component({
  ...
  exportAs: 'akaVerticalNavigation',
})
export class AkaVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
  static ngAcceptInputType_inner: BooleanInput;
  static ngAcceptInputType_opened: BooleanInput;
  static ngAcceptInputType_transparentOverlay: BooleanInput;

  @Input() appearance: AkaVerticalNavigationAppearance = 'default';
  @Input() autoCollapse = true;
  @Input() inner = false;
  @Input() mode: AkaVerticalNavigationMode = 'side';
  ...
  @Output()
  readonly positionChanged: EventEmitter<AkaVerticalNavigationPosition> = new EventEmitter<AkaVerticalNavigationPosition>();
  @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

  activeAsideItemId: string | null = null;
  onCollapsableItemCollapsed: ReplaySubject<AkaNavigationItem> = new ReplaySubject<AkaNavigationItem>(1);
  onCollapsableItemExpanded: ReplaySubject<AkaNavigationItem> = new ReplaySubject<AkaNavigationItem>(1);
  ...
  private _akaScrollbarDirectives!: QueryList<AkaScrollbarDirective>;
  private _akaScrollbarDirectivesSubscription: Subscription;
  ...
  @HostBinding('class') get classList(): any {
    return {
      'aka-vertical-navigation-animations-enabled': this._animationsEnabled,
      [`aka-vertical-navigation-appearance-${this.appearance}`]: true,
      'aka-vertical-navigation-hover': this._hovered,
      'aka-vertical-navigation-inner': this.inner,
      'aka-vertical-navigation-mode-over': this.mode === 'over',
      'aka-vertical-navigation-mode-side': this.mode === 'side',
      'aka-vertical-navigation-opened': this.opened,
      'aka-vertical-navigation-position-left': this.position === 'left',
      'aka-vertical-navigation-position-right': this.position === 'right',
    };
  }
  ...
  /**
   * Setter for akaScrollbarDirectives
   */
  @ViewChildren(AkaScrollbarDirective)
  set akaScrollbarDirectives(akaScrollbarDirectives: QueryList<AkaScrollbarDirective>) {
    // Store the directives
    this._akaScrollbarDirectives = akaScrollbarDirectives;

    // Return if there are no directives
    if (akaScrollbarDirectives.length === 0) {
      return;
    }

    // Unsubscribe the previous subscriptions
    if (this._akaScrollbarDirectivesSubscription) {
      this._akaScrollbarDirectivesSubscription.unsubscribe();
    }

    // Update the scrollbars on collapsable items' collapse/expand
    this._akaScrollbarDirectivesSubscription = merge(this.onCollapsableItemCollapsed, this.onCollapsableItemExpanded)
      .pipe(takeUntil(this._unsubscribeAll), delay(250))
      .subscribe(() => {
        // Loop through the scrollbars and update them
        akaScrollbarDirectives.forEach((akaScrollbarDirective) => {
          akaScrollbarDirective.update();
        });
      });
  }
  ...
  ngAfterViewInit(): void {
    setTimeout(() => {
      ...
      if (!this._navigationContentEl.nativeElement.classList.contains('ps')) {
        ...
        const activeItem = this._navigationContentEl.nativeElement.querySelector('.aka-vertical-navigation-item-active');
      ...
      // Otherwise
        ...
        this._akaScrollbarDirectives.forEach((akaScrollbarDirective) => {
          // Skip if not enabled
          if (!akaScrollbarDirective.isEnabled()) {
            return;
          }

          // Scroll to the active element
          akaScrollbarDirective.scrollToElement('.aka-vertical-navigation-item-active', -120, true);
        });
      }
    });
  }
   ...
   * @param item
   */
  openAside(item: AkaNavigationItem): void {
   ...
   * @param item
   */
  toggleAside(item: AkaNavigationItem): void {
    // Toggle
  ...
  private _showOverlay(): void {
    ...
    this._overlay.classList.add('aka-vertical-navigation-overlay');

    // Add a class depending on the transparentOverlay option
    if (this.transparentOverlay) {
      this._overlay.classList.add('aka-vertical-navigation-overlay-transparent');
    }
  ...
  private _showAsideOverlay(): void {
    ...
    this._asideOverlay.classList.add('aka-vertical-navigation-aside-overlay');
