...
import { ScrollbarGeometry, ScrollbarPosition } from '@aka/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
  selector: '[akaScrollbar]',
  exportAs: 'akaScrollbar',
})
export class AkaScrollbarDirective implements OnChanges, OnInit, OnDestroy {
  static ngAcceptInputType_akaScrollbar: BooleanInput;

  @Input() akaScrollbar = true;
  @Input() akaScrollbarOptions: PerfectScrollbar.Options;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // Enabled
    if ('akaScrollbar' in changes) {
      // Interpret empty string as 'true'
      this.akaScrollbar = coerceBooleanProperty(changes.akaScrollbar.currentValue);

      // If enabled, init the directive
      if (this.akaScrollbar) {
        this._init();
      }
    ...
    // Scrollbar options
    if ('akaScrollbarOptions' in changes) {
      // Merge the options
      this._options = merge({}, this._options, changes.akaScrollbarOptions.currentValue);
