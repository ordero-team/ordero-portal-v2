...
import { AkaMockApiService } from '@aka/lib/mock-api/mock-api.service';

@Injectable({
  providedIn: 'root',
})
export class AkaMockApiInterceptor implements HttpInterceptor {
  /**
   * Constructor
   */
  constructor(
    @Inject(AKA_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
    private _akaMockApiService: AkaMockApiService
  ) {}
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    const { handler, urlParams } = this._akaMockApiService.findHandler(request.method.toUpperCase(), request.url);
