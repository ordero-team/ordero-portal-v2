...
export class OrderService {
  private orders: BehaviorSubject<OwnerOrder[]> = new BehaviorSubject([]);
  ...
  setData(data: OwnerOrder[]) {
    this.orders.next(data);
  }

  add(order: OwnerOrder) {
  ...
  updateOrder(orderId: string, updatedOrder: Partial<OwnerOrder>) {
    const currentOrders = this.orders.value;
