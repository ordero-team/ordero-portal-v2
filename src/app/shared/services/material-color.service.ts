...
export class MaterialColorService {
  ...
  states: any = {
    ...
    processing: this.colors.blue[900],
    active: this.colors.green,
    inactive: this.colors.grey,
    blocked: this.colors.red[700],
  ...
  styleOf(state: string): IStateStyle {
    ...
    return color
      ? {
          color: isObject(color) ? color[500] : color,
          background: isObject(color) ? color[50] : this.newShade(color, 165),
          border: `1px solid ${isObject(color) ? color[500] : color}`,
        }
      : {
          color: this.colors[`grey-500`],
          background: this.colors[`grey-50`],
        };
  }
